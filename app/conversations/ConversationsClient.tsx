...
function ConversationsContent() {
  ...
  const [selectedHandoff, setSelectedHandoff] = useState<HandoffRecord | null>(
    null,
  );
  const [isDetailLoading, setIsDetailLoading] = useState(false);

  const [selectedMock, setSelectedMock] = useState<MockConversation | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const activeFilter: FilterValue =
      urlFilter &&
      ["new", "pending", "resolved", "awaiting-payment", "all"].includes(
        urlFilter,
      )
        ? urlFilter
        : "all";
  ...
  async function handleMarkResolved() {
    ...
    try {
      console.log("selectedHandoff.id =>", selectedHandoff.id);
  ...
  return (
    <>
      <div
        className={`w-full md:w-80 lg:w-96 border-r border-gray-200 bg-white flex flex-col shrink-0 ${
          isMobileDetailOpen ? "hidden md:flex" : "flex"
        }`}
      >
        {/* Header */}
        <div className="p-4 lg:p-5 border-b border-gray-200">
          <h1 className="text-xl lg:text-2xl font-semibold text-gray-900 mb-3">
            Conversations
          </h1>
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-1.5 pb-1 whitespace-nowrap">
            {(["all", "pending", "awaiting-payment", "resolved"] as const).map(
              (f) => {
                const label =
                  f === "awaiting-payment"
                    ? "Payment"
      ...
      <div
        className={`flex-1 bg-gray-50 flex flex-col min-w-0 ${
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-3 mb-2">
                          <h2 className="text-xl sm:text-2xl font-semibold text-gray-900 leading-tight">
                            {selectedHandoff.customerName ??
                              selectedHandoff.waId}
                          </h2>
                      ...
                      <div className="flex flex-wrap gap-2 shrink-0">
                        {selectedHandoff.status !== "resolved" && (
                          ...
                          >
                            ...
                            <span className="hidden xl:inline">Mark as </span>
                            Resolved
                          </button>
                        )}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs sm:text-sm text-gray-600 border-t border-gray-100 pt-3 mt-3 sm:border-0 sm:pt-0 sm:mt-0">
                      <div>
                        ...
                        <span className="capitalize">
                          {selectedHandoff.category.replace(/_/g, " ")}
                        </span>
                      </div>
                ...
                <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6 space-y-3 lg:space-y-4">
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tfoot>
                          <tr className="border-t-2 border-gray-200">
                            <td
                              colSpan={3}
                              className="pt-2 font-semibold text-gray-900"
                            >
                              Total
                            </td>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 p-4 lg:p-5">
                    ...
                    <div className="space-y-2.5 mb-3">
                      {localComments[selectedHandoff.id] &&
                      localComments[selectedHandoff.id].length > 0 ? (
                        localComments[selectedHandoff.id].map((comment) => (
                          <div
                            key={comment.id}
                            className="bg-amber-50 border border-amber-200 rounded-lg p-2.5 sm:p-3"
                          >
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 lg:p-6 space-y-3 lg:space-y-4">
                            ...
                            <div className="bg-white rounded p-2 sm:p-2.5 text-xs sm:text-sm">
                              ...
                              {message.metadata.type === "erp" && (
                                <div className="space-y-1 text-gray-700">
                                  ...
                                  <p>
                                    Available: {message.metadata.data.available}{" "}
                                    units
                                  </p>
                                  <p>
                                    Unit Price: $
                                    {message.metadata.data.unitPrice}
                                  </p>
                                  <p className="font-semibold">
                                    Total: $
                                    {message.metadata.data.totalPrice.toFixed(
                                      2,
                                    )}
                                  </p>
                                </div>
                              ...
                              {message.metadata.type === "payment" && (
                                <div className="space-y-1 text-gray-700">
                                  <p>
                                    Order ID: {message.metadata.data.orderId}
                                  </p>
                                  ...
                                  <p>
                                    Attempts: {message.metadata.data.attempts}
                                  </p>
                                </div>
                              )}
                              {message.metadata.type === "delivery" && (
                                <div className="space-y-1 text-gray-700">
                                  ...
                                  <p>
                                    Address: {message.metadata.data.address}
                                  </p>
                                  <p>
                                    Special:{" "}
                                    {message.metadata.data.specialInstructions}
                                  </p>
                                  <p>
                                    Est. Delivery:{" "}
                                    {message.metadata.data.estimatedDelivery}
                                  </p>
                                </div>
                              )}
                            </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4 lg:p-5">
                ...
                <div className="space-y-2.5 mb-3">
                  ...
                  (localComments[selectedMock.id] &&
                    localComments[selectedMock.id].length > 0) ? (
                    <>
